...
function getCalendarLink(dateStr, timeSlot) {
    ...
    const details = encodeURIComponent(`加油！別遲到囉！`);
    const location = encodeURIComponent(`小野人餐盒製造所 (Dade店)`);
    
    let startTime = "090000", endTime = "140000"; // 預設早班
    if (timeSlot.includes("晚") || timeSlot.includes("17")) { startTime = "170000"; endTime = "210000"; }
    
    return `https://www.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${fullDate}T${startTime}/${fullDate}T${endTime}&details=${details}&location=${location}&sf=true&output=xml`;
}
...
function render(data, title, type = 'schedule') {
    ...
    data.forEach(item => {
        ...
        const calBtn = (!isOff && type === 'schedule' && n === currentUser) ? 
            `<a href="${getCalendarLink(d, s)}" target="_blank" style="color: #3498db; margin-left: 10px; font-size: 1.2em;"><i class="fa-regular fa-calendar-plus"></i></a>` : "";

        card.innerHTML = `
            <div style="flex: 1;">
                <div style="font-weight:bold; display: flex; align-items:center; color:#2c3e50;">${d} ${calBtn}</div>
            ...
            <div style="font-weight:bold; color:#34495e;">${n}</div>
        `;
...
// 4. 切換頁面
function showToday() {
    currentTab = "today"; updateActive(0);
...
function showThisWeek(m = "all") {
    currentTab = "week"; updateActive(1);
